'''
 Magnetic fitting for Triple 5 superlattice measure on Platypus in 1T field
Program: Refl1D Program

Fit strategy: Use FreeMagnetism to split LSMO layer into three parts like in Hoffman paper. Am fitting:

LSMO rhoM for z = 1, 2, 3
LNO rhoM
...
LSMO_lyrs_mag = [ FreeMagnetism( z = np.linspace(0,1,3), rhoM = np.full(3,5), thetaM = np.full(3,90), name="LSMO_mag_" + str(i) ) for i in range(1,16)]
LNO_lyrs_mag1 = [ Magnetism( rhoM = 0.2, thetaM = 90, name="LNO_3uc_bottom_mag_" + str(i) ) for i in range(1,6) ]
LNO_lyrs_mag2 = [ Magnetism( rhoM = 0.2, thetaM = 90, name="LNO_1uc_mag_" + str(i) ) for i in range(1,6) ]
LNO_lyrs_mag3 = [ Magnetism( rhoM = 0.2, thetaM = 90, name="LNO_3uc_top_mag_" + str(i) ) for i in range(1,6) ]

sample = STO( 0, 1.5 ) | LSMO( 
    p[2], p[0], LSMO_lyrs_mag[0] ) | LNO( 
    p[5], p[3], LNO_lyrs_mag1[0] ) | LSMO( 
    p[7], p[6], LSMO_lyrs_mag[1] ) | LNO( 
    p[9], p[8], LNO_lyrs_mag1[1] ) | LSMO( 
    p[11], p[10], LSMO_lyrs_mag[2] ) | LNO( 
    p[13], p[12], LNO_lyrs_mag1[2] ) | LSMO( 
    p[15], p[14], LSMO_lyrs_mag[3] ) | LNO( 
    p[17], p[16], LNO_lyrs_mag1[3] ) | LSMO(
    p[19], p[18], LSMO_lyrs_mag[4] ) | LNO(
    p[21], p[20], LNO_lyrs_mag1[4] ) | LSMO(
    p[23], p[22], LSMO_lyrs_mag[5] ) | LNO(
    p[25], p[24], LNO_lyrs_mag2[0] ) | LSMO(
    p[27], p[26], LSMO_lyrs_mag[6] ) | LNO(
    p[29], p[28], LNO_lyrs_mag2[1] ) | LSMO(
    p[31], p[30], LSMO_lyrs_mag[7] ) | LNO(
    p[33], p[32], LNO_lyrs_mag2[2] ) | LSMO(
    p[35], p[34], LSMO_lyrs_mag[8] ) | LNO(
    p[37], p[36], LNO_lyrs_mag2[3] ) | LSMO(
    p[39], p[38], LSMO_lyrs_mag[9] ) | LNO(
    p[41], p[40], LNO_lyrs_mag2[4] ) | LSMO(
